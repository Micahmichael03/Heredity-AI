...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    joint_prob = float(1)
    for person in people:
        person_gene = 1 if person in one_gene else 2 if person in two_genes else 0          # Get info about person (gene number, trait, mother/father)
        person_trait = True if person in have_trait else False
        person_trait_prob = PROBS['trait'][person_gene][person_trait]
        mother = people[person]['mother']
        father = people[person]['father']
        
        if mother is None and father is None:
            # For anyone with no parents listed in the data set, 
            # use the probability distribution PROBS["gene"] to determine the probability
            prob = PROBS['gene'][person_gene]
        else:
            ...
            mother_prob = 0.01 if mother_gene == 0 else 0.5 if mother_gene == 1 else 0.99
            father_prob = 0.01 if father_gene == 0 else 0.5 if father_gene == 1 else 0.99
        ...
        joint_prob *= prob * person_trait_prob
    ...
    for person in probabilities:
        for dist in probabilities[person]:
            ...
            for value in probabilities[person][dist]:
                probabilities[person][dist][value] /= total
